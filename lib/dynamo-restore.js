...
DynamoRestore.prototype._startDownload = function() {
    ...
    var createReadline(stream, meta) {
        ...
        self.readline = readline.createInterface({
            ...
            .on('close', function() {
                ...
                self._sendBatch(self.requestItems.splice(0, DYNAMO_CHUNK_SIZE));
                self._finishBatches();
            });
    ...
    if (options.localfile) {
        ...
        meta = {RemainingLength: fs.statSync(options.source).size};
...
DynamoRestore.prototype._sendBatch = function(items) {
    ...
    var params = { RequestItems: {} },
        ...
        options = this.options,
        self = this;
    var interval = 1000 / options.maxConcurrency;
    params.RequestItems[options.table] = items;

    // Send
    var waitForConcurrency = function() {
        if (self.requests < options.concurrency) {
            self.emit('send-batch', options.concurrency, self.requests, self.readline.meta);
            self.requests++;
            start = Date.now();
            return dynamo.batchWriteItem(params, function(error, data) {
                ...
                } else if (Date.now() - start < interval){
                    if (options.concurrency > options.minConcurrency) {
                        options.concurrency--;
                    }
                } else {
            ...
            self.readline.pause();
            setTimeout(waitForConcurrency, 100);
        }
    };
    waitForConcurrency();
};
