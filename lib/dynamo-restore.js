...
DynamoRestore.prototype.run = function(finishCallback) {
    ...
    this.on('finish', (function() {
        ...
        if (options.writecapacity) {
            dynamodb.describeTable({TableName: this.options.table}, (function(err, data) {
                if (err || !data) {
                    return this.emit('error', 'Fatal Error. Failed describing table. ' + err);
                }
                gsiParams = [];
                for (p in data['Table']['GlobalSecondaryIndexes']) {
                    indexName = data['Table']['GlobalSecondaryIndexes'][p]['IndexName'];
                    gsiParams.push({
                        Update: {
                            IndexName: indexName,
                ...
                if (gsiParams.length > 0) {
                    params['GlobalSecondaryIndexUpdates'] = gsiParams;
                }
                ...
                dynamodb.updateTable(params, (function(error, data) {
                    if (error || !data) {
                        retry = true;
                        return this.emit('warning', 'WARN: FAILED TO UPDATE TABLE. ' + error);
                    }
                    finishCallback();
                }).bind(this));
                setTimeout((function() { if (retry) this.emit('finish') }).bind(this), 600);
            }).bind(this));
        } else {
            finishCallback();
        }
...
DynamoRestore.prototype._validateS3Backup = function(options) {
    ...
    options.s3schemapath = url.pathname.split('.')[0] + '.schema.json';
    options.s3schemapath = options.s3schemapath.substr(1);
...
DynamoRestore.prototype._checkTableExists = function(error, data) {
    ...
    if (data.TableNames.indexOf(this.options.table) > -1) {
        ...
        if (this.options.overwrite) {
            ...
            dynamodb.describeTable({TableName: this.options.table}, (function(err, data) {
                if (error || !data) {
                    return this.emit('error', 'Fatal Error. Failed describing table. ' + error);
                }
                gsiParams = [];
                for (p in data['Table']['GlobalSecondaryIndexes']) {
                    indexName = data['Table']['GlobalSecondaryIndexes'][p]['IndexName'];
                    gsiParams.push({
                        Update: {
                            ...
                            ProvisionedThroughput: {
                                ReadCapacityUnits: options.readcapacity,
                                WriteCapacityUnits: options.indexConcurrency
                            }
                        }
                    });
                }
...
DynamoRestore.prototype._startDownload = function() {
    ...
    // fetch schema to recreate table
    if (!this.options.overwrite) {
        var params = {
            Bucket: this.options.s3bucket,
            Key: this.options.s3schemapath
        };
        s3.getObject(params, (function(error, data) {
            if (error) {
                if (error.code === 'NotFound') {
                    this.emit('error', util.format('Could not find file schema for %s', this.options.source));
                } else {
                    this.emit('error', util.format('Error downloading file from s3: %s', error));
                }
                return;
            }
            this.options.schema = JSON.parse(data.Body.toString('utf-8'));
        }).bind(this));
    }
...
DynamoRestore.prototype._createTable = function(callback) {
    ...
    // we want to create a table but we still don't have the schema
    if (!options.overwrite && !options.schema) {
        this.emit('warning', 'Schema not read yet. Waiting a bit...')
        setTimeout(this._createTable.bind(this), 1000);
    }
    ...
    // we have a schema read from a file, override params
    if (options.schema) {
        // set GSI troughtput
        for (var p in options.schema.GlobalSecondaryIndexes) {
            options.schema.GlobalSecondaryIndexes[p].ProvisionedThroughput.ReadCapacityUnits = options.readcapacity;
            options.schema.GlobalSecondaryIndexes[p].ProvisionedThroughput.WriteCapacityUnits = options.concurrency;
        }

        params = {
            TableName: options.table,
            AttributeDefinitions: options.schema.AttributeDefinitions,
            KeySchema: options.schema.KeySchema,
            ProvisionedThroughput: {
                ReadCapacityUnits: options.readcapacity,
                WriteCapacityUnits: options.concurrency // Need this high for pumping data, but will reduce it later.
            },
    ...
    dynamodb.createTable(params, (function(error, data) {
        ...
        setTimeout(dynamodb.describeTable.bind(dynamodb, { TableName: data.TableName }, this._checkTableReady.bind(this)), 10000);
    }).bind(this));
};
...
DynamoRestore.prototype._checkTableReady = function(error, data) {
    ...
    if (data && data.Table && data.Table.TableStatus === 'ACTIVE') {
...
DynamoRestore.prototype._finishBatches = function() {
    if (!this.batches.length) {
        ...
        if (this.requests.length < this.options.concurrency) {
            while (this.batches.length && this.requests.length < 25) {
                this._sendBatch();
            }
        }
    }
