...
DynamoRestore.prototype._verifyBackupFilesExist = function (callback) {
    ...
    function checkSchemaFile (done) {
        ...
        self.emit('info', `Querying for existance of schema: "${schemaPath}" in the "${self.options.bucket}" bucket.`)
    ...
    function checkBackupFile (done) {
        ...
        self.emit('info', `Querying for existance of backup: "${backupPath}" in the "${self.options.bucket}" bucket.`)
