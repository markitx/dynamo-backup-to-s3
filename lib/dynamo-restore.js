...
DynamoRestore.prototype._validateTable = function(options) {
	...
	dynamodb.listTables({}, this._checkExists.bind(this));
};

DynamoRestore.prototype._checkExists = function(error, data) {
	this.debug('DynamoRestore.prototype._checkExists()', data);
	...
	if (this.options.partitionkey && this.options.partitionkeytype) {
		...
		// Not enough info about the partition key! 
		// Thats ok, we can use the s3 data to generate.
		if (this.requestItems && this.requestItems[0] && this.requestItems[0].PutRequest) {
			var template = this.requestItems[0].PutRequest.Item;
			if (template) {
				this._getPartitionKeyInfo(template);
				//this.options.partitionkeytype = this._getPartition
				//this.options.sortkey = undefined;
			}
		}
		// No backup info? Loop until we have some
		setTimeout(this._checkExists.bind(this, error, data), 200);
	}
};

DynamoRestore.prototype._getPartitionKeyInfo = function(template) {
	this.debug('DynamoRestore.prototype._getPartitionKeyInfo(template)', template);
	...
	if (partitionkey) {
		...
		var likelyCandidates = Object.keys(template).filter(function(column) {
			console.log('column "%s"', column.toLowerCase());
			return column.toLowerCase().substr(-2) === 'id'
				||	column.toLowerCase().substr(-3) === 'key'
				||	column.toLowerCase().substr(-4) === 'href';
		});
		if (likelyCandidates.length === 0) {
			this.emit('error', 'Fatal Error. Cannot determine --partitionkey from backup, please supply explicitly.');
		} else {
...
DynamoRestore.prototype._createTable = function() {
...
DynamoRestore.prototype._checkTableReady = function(error, data) {
	...
	if (data 
		...
		&& data.Table.TableStatus === 'ACTIVE') {
			// Signal ready and kick off batch queue.
			this.tableready = true;
			while (this.batches.length && this.requests.length < this.options.concurrency) {
				this._sendBatch();
			}
	} else {
...
DynamoRestore.prototype._streamBackup = function() {
	...
	s3.headObject(params, (function (error, meta) {
		...
		var throttledStream = this._throttleStream(s3.getObject(params).createReadStream());
		readline
			.createInterface({ input: throttledStream })
...
DynamoRestore.prototype._throttleStream = function(stream) {
	this.debug('DynamoRestore.prototype._throttleStream()');
...
DynamoRestore.prototype._processLine = function(line) {
	...
	// Check we are not exceeding concurrency (ie max requests) before sending another batch
	if (this.tableready && this.batches.length && this.requests.length < this.options.concurrency) {
		this._sendBatch();
	}
};
...
DynamoRestore.prototype._finishBatches = function() {
	...
	if (!this.batches.length) {
		...
		if (this.tableready && this.requests.length < this.options.concurrency) {
			this._sendBatch();
		}
	}
