...
DynamoRestore.prototype._convertSchemaToRequestParams = function () {
    ...
    self.emit('info', `Setting initial WriteCapacityUnits to: ${self.options.concurrency}`)
...
DynamoRestore.prototype._sendBatch = function () {
    ...
    self.requests.push(self.dynamodb.batchWriteItem(params, function (error, data) {
        ...
        if (error) {
            ...
            if (batch.attempts > self.options.concurrency) {
                if (self.options.stopOnFailure) {
                    ...
                    self.emit('debug', 'Failed to upload same batch too many times, removing from queue.. \n' + JSON.stringify(batch))
                }
            } else {
                self.emit('debug', 'Error processing batch, putting back in the queue.')
                batch.attempts++
        ...
        if (unprocessedItems.length) {
            ...
            self.emit('debug', `${unprocessedItems.length} unprocessed items. Add to queue and back off a bit.`)
            self.batches.push({
            ...
            const progressPercent = (self.restoredLength / self.contentLength * 100).toFixed(3)
