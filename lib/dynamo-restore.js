...
DynamoRestore.prototype.run = function () {
    ...
    self._verifyBackupFilesExist(error => {
        if (error) return emitErrorAndFinish(error)
        ...
        self._fetchSchema((error, { Body }) => {
            ...
            try {
                ...
                return emitErrorAndFinish(error)
            }
...
DynamoRestore.prototype._verifyBackupFilesExist = function (callback) {
    ...
    function checkBackupFile () {
        ...
        self.s3.headObject(params, (error, metadata) => {
            ...
            self.remainingLength = metadata.ContentLength
            return callback(null, metadata)
        })
    }

    async.parallel([checkSchemaFile, checkBackupFile], callback)
}
...
DynamoRestore.prototype._startDownload = function (callback) {
    ...
    self.readline = readline.createInterface({ terminal: false, nput: downloadStream })
...
DynamoRestore.prototype._processLine = function (line) {
    ...
    self.remainingLength -= line.length + 1
...
DynamoRestore.prototype._sendBatch = function () {
    ...
    // Send
    self.requests.push(self.dynamo.batchWriteItem(params, function (error, data) {
        self.requests.shift()
        if (error) {
    ...
    self.emit('send-batch', self.batches.length, self.requests.length)
}
