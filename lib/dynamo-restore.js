...
function DynamoRestore(options) {
  ...
  options.awsRegion =
    options.awsRegion || process.env.AWS_DEFAULT_REGION || "ap-southeast-2";
...
DynamoRestore.prototype._validateS3Backup = function (options) {
  ...
  if (!url || url.protocol !== "s3:") {
    return this.emit(
      "error",
      "Please provide an s3 URI as file source (ie s3://mybucketname/folder/mydynamotable.json)"
    );
  }
  if (!url.pathname || !url.hostname || url.search || url.hash || url.auth) {
    return this.emit(
      "error",
      "Please provide a simple s3 URI as file source (ie s3://mybucketname/folder/mydynamotable.json)"
    );
  }
  if (url.pathname.substr(-5).toLowerCase() !== ".json") {
    return this.emit(
      "error",
      "Please provide a *.json file as source restoring backup."
    );
  }
...
DynamoRestore.prototype._validateTable = function (options) {
  ...
  if (!options.table) {
    return this.emit(
      "error",
      "Please provide a Dynamo DB table name to restore to."
    );
  }
...
DynamoRestore.prototype._checkTableExists = function (error, data) {
  ...
  if (error || !data || !data.TableNames) {
    return this.emit(
      "error",
      "Fatal Error. Could not connect to AWS DynamoDB engine. Please check your credentials."
    );
  }
  if (data.TableNames.indexOf(this.options.table) > -1) {
    ...
    if (this.options.overwrite) {
      this.emit(
        "warning",
        util.format("WARN: table [%s] will be overwritten.", this.options.table)
      );
      setTimeout(
        dynamodb.describeTable.bind(
          dynamodb,
          { TableName: this.options.table },
          this._checkTableReady.bind(this)
        ),
        1000
      );
    } else {
      this.emit(
        "error",
        "Fatal Error. The destination table already exists! Exiting process.."
      );
    }
    return;
  }
    ...
    function (error, meta) {
      if (error) {
        if (error.code === "NotFound")
          this.emit(
            "error",
            util.format("Could not find file in s3. %s", this.options.source)
          );
        else
          this.emit(
            "error",
            util.format("Error downloading file from s3: %s", error)
          );
        return;
      }
...
DynamoRestore.prototype._extractSchema = function (template) {
  ...
  if (partitionkey) {
    ...
    if (!template[partitionkey]) {
      return this.emit(
        "error",
        util.format(
          'Fatal Error. The --partitionkey "%s" you provided is not a valid column.',
          partitionkey
        )
      );
    }
    ...
    var likelyCandidates = Object.keys(template).filter(function (column) {
      return (
        column.toLowerCase().substr(-2) === "id" ||
    ...
    if (likelyCandidates.length === 0) {
      return this.emit(
        "error",
        "Fatal Error. Cannot determine --partitionkey from backup, please supply it manually."
      );
    } else {
      // Pick the shortest one
      partitionkey = likelyCandidates
        .sort(function (a, b) {
  ...
  if (this.options.sortkey) {
    ...
    if (!template[sortkey]) {
      return this.emit(
        "error",
        "Fatal Error. The --sortkey you provided is not available for some records."
      );
    }
...
DynamoRestore.prototype._createTable = function (callback) {
  ...
  if (!options.table || !options.partitionkey) {
    return this.emit(
      "error",
      "Fatal Error. Could not create dynamo table. Not enough information provided."
    );
  }
  var params = {
    TableName: options.table,
  ...
  dynamodb.createTable(
    ...
    function (error, data) {
      if (error || !data) {
        return this.emit(
          "error",
          "Fatal Error. Failed to create new table. " + error
        );
      }
      ...
      function (error, data) {
        ...
        if (error) {
          ...
          if (batch.attempts > options.concurrency) {
            if (options.stopOnFailure) {
              return this.emit(
                "error",
                "Fatal Error. Failed to upload batch. Ending process. \n" +
                  JSON.stringify(batch)
              );
            } else {
              this.emit(
                "warning",
                "Failed to upload same batch too many times, removing from queue.. \n" +
                  JSON.stringify(batch)
              );
            }
          } else {
        ...
        var unprocessedItems =
          (data &&
            ...
            data.UnprocessedItems[options.table]) ||
          [];
        if (unprocessedItems.length) {
          ...
          this.emit(
            ...
            unprocessedItems.length +
              " unprocessed items. Add to queue and back off a bit."
          );
          this.batches.push({
        ...
        if (
          this.tableready &&
          this.requests.length < options.concurrency * 0.8
        ) {
          this.readline.resume();
        }
