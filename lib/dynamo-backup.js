...
DynamoBackup.prototype._streamItems = function fetchItems(tableName, startKey, limit, itemsReceived, callback) {
    ...
    self.ddb.scan(params, function(err, data) {
        if (err) {
            return callback(err);
        }
        ...
        self._streamItems(data.LastEvaluatedKey, limit, itemsReceived, callback);
    });
}
...
DynamoBackup.prototype._fetchTables = function(lastTable, tables, callback) {
    ...
    self.ddb.listTables(params, function(err, data) {
        if (err) {
