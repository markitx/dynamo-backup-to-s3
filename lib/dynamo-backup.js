...
function DynamoBackup(options) {
    ...
    this.awsAccessKey = options.awsKey || process.env.AWS_ACCESS_KEY_ID;
    this.awsSecretKey = options.awsSecret || process.env.AWS_SECRET_ACCESS_KEY;
...
DynamoBackup.prototype.backupTable = function (tableName, backupPath, callback) {
   ...
   if (self.awsAccessKey && self.awsSecretKey) {
    ...
    if (self.awsSessionToken) {
        params.sessionToken = self.awsSessionToken;
    }

   params.bucket = self.bucket;
   params.objectName = `${backupPath}/${tableName}.json`;
...
DynamoBackup.prototype._copyTable = function (tableName, itemsReceived, callback) {
    ...
    ddb.describeTable({ TableName: tableName }, function (err, data) {
        ...
        if (data.Table.BillingModeSummary && data.Table.BillingModeSummary.BillingMode === 'PAY_PER_REQUEST') { 
