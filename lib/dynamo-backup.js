...
DynamoBackup.prototype.backupTable = function (tableName, backupPath, callback) {
   ...
   var upload = new Uploader(params);
    ...
    // ends the stream, prints an error message if given
    ...
    if (self.readCapacity === 0) {
        self._copyTable(tableName, self.parallel, itemsHandler, errorCallback);
    } else {
        self._updateTable(
            tableName,
            self.readCapacity,
            function (oldReadCapacity, oldWriteCapacity) {
                self._copyTable(tableName, self.parallel, itemsHandler, function(err) {
                    self._restoreTable(tableName, oldReadCapacity, oldWriteCapacity, errorCallback);
                    errorCallback(err);
                });
            },
            errorCallback
...
DynamoBackup.prototype._copyTable = function (tableName, parallelReads, itemsReceived, callback) {
    var self = this;
    ...
    ddb.describeTable({ TableName: tableName }, function (err, data) {
        ...
        if (parallelReads > 1) {
            async.map(
                ...
                async function(i, callback) {
                    self._streamItems(tableName, null, limit, i, parallelReads, itemsReceived, callback);
                },
                callback
            );
        } else {
...
DynamoBackup.prototype._updateTable = function (tableName, newReadCapacity, successCallback, errorCallback) {
    ...
    ddb.describeTable({ TableName: tableName }, function (err, data) {
        ...
        ddb.updateTable(params, function (err, data) {
            ...
            successCallback(oldReadCapacity, oldWriteCapacity);
        });
    })
}
