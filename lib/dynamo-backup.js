...
DynamoBackup.prototype.backupTable = function (tableName, backupPath, callback) {
   ...
   var upload = new Uploader(params);
    ...
    if (self.concurrency === 0) {
        self._copyTable(tableName, itemsHandler, errorCallback);
    } else {
        self._updateTable(
            tableName,
            self.concurrency,
            function (oldReadCapacity, oldWriteCapacity) {
                self._copyTable(tableName, itemsHandler, function(err) {
...
// starts a _streamItems() loop, which calls itemsReceived each time, then callback when finished or error
