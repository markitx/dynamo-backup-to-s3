...
function DynamoBackup(options) {
    ...
    this.saveSchema = options.saveSchema || true;
...
DynamoBackup.prototype.backupTable = function (tableName, backupPath, callback) {
    var self = this;
    var stream = new ReadableStream({ highWaterMark: 5000000 });
    ...
    upload.send(function (err) {
        ...
        if (self.saveSchema) {
            ...
            self._copyTableSchema(tableName, schemaParams, function (err, data) {
...
DynamoBackup.prototype._copyTableSchema = function (tableName, params, callback) {
    var self = this;
    var ddb = new AWS.DynamoDB();
    var s3 = new AWS.S3();
    ddb.describeTable({ TableName: tableName }, function (err, data) {
        ...
        params.Body = JSON.stringify(self._cleanupSchema(data.Table));
...
DynamoBackup.prototype._cleanupSchema = function (schema) {
    ...
    if (schema.GlobalSecondaryIndexes) {
        for (var p in schema.GlobalSecondaryIndexes) {
            delete schema.GlobalSecondaryIndexes[p].ProvisionedThroughput.NumberOfDecreasesToday;
            delete schema.GlobalSecondaryIndexes[p].ProvisionedThroughput.LastIncreaseDateTime;
            delete schema.GlobalSecondaryIndexes[p].ProvisionedThroughput.LastDecreaseDateTime;
            delete schema.GlobalSecondaryIndexes[p].CreationDateTime;
            delete schema.GlobalSecondaryIndexes[p].ItemCount;
            delete schema.GlobalSecondaryIndexes[p].TableSizeBytes;
            delete schema.GlobalSecondaryIndexes[p].IndexArn;
            delete schema.GlobalSecondaryIndexes[p].IndexStatus;
            delete schema.GlobalSecondaryIndexes[p].IndexSizeBytes;
        }
    }
